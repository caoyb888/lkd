...
// forked from design-sync lib/source-kit.mjs — exclude app-entry files (main/App)
// from the synth-entry star list and from component discovery: this is a Vite
// app (no library entry), so main.tsx renders on import and App.tsx mounts the
// whole router — neither is a design-system component.
...
// App-entry / side-effect files that must never be bundled or listed as components.
const APP_ENTRY_RX = /(?:^|\/)(?:main|App)\.(?:tsx|jsx)$/;
const APP_ENTRY_NAMES = new Set(['App', 'Main']);
...
// Synth entry built by `export *` drops DEFAULT exports — this Vite app's
// Views/layout/composite are all `export default function <Name>()`, so they'd
// be discovered but missing from window.<GLOBAL>. Emit an explicit
// `export { default as <Name> }` for every file with a named default export so
// they land on the global bundle alongside the named exports.
function synthEntrySource(compFiles) {
  const project = new Project({
    skipAddingFilesFromTsConfig: true,
    compilerOptions: { jsx: ts.JsxEmit.Preserve, allowJs: true, skipLibCheck: true },
  });
  // Pass 1: collect every PascalCase NAMED export (these already reach the
  // global via `export *`), plus each file's default-export name. A name that
  // exists as a named export anywhere must NOT also get a default re-export —
  // e.g. the real `DataTable` is a named export in ui/data-table.tsx, while a
  // legacy components/DataTable.tsx default-exports a placeholder of the same
  // name; preferring the named export keeps the right component on the global.
  const namedExports = new Set();
  const defaults = [];
  for (const p of compFiles) {
    const sf = project.addSourceFileAtPathIfExists(p);
    if (!sf) continue;
    for (const [key] of sf.getExportedDeclarations()) {
      if (key !== 'default' && /^[A-Z][A-Za-z0-9]*$/.test(key)) namedExports.add(key);
    }
    const decls = sf.getExportedDeclarations().get('default');
    const name = decls?.map((d) => d.getName?.()).find((n) => n && n !== 'default');
    if (name && /^[A-Z][A-Za-z0-9]*$/.test(name)) defaults.push({ p, name });
  }
  // Pass 2: `export *` for named exports, then explicit default re-exports for
  // default-only components (skipping any name that's already a named export).
  const lines = compFiles.map((p) => `export * from ${JSON.stringify(p)};`);
  const usedNames = new Set();
  for (const { p, name } of defaults) {
    if (namedExports.has(name) || usedNames.has(name)) continue;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    writeFileSync(entry, synthEntrySource(comps));
    synthEntry = true;
    console.error(
  ...
  if (!components.length && synthEntry) {
    components = deriveComponentsFromSrc(srcFiles).filter(
      (c) => srcMap[c.name] !== null && !APP_ENTRY_NAMES.has(c.name),
    );
  }
